...
def threshold_predict(x: np.ndarray, threshold: float) -> np.ndarray:
    """Return binary labels by thresholding the input values.

    Elements of `x` greater than `threshold` are labeled as `1`, otherwise `0`.

    Parameters
    ----------
    x : np.ndarray
        Array of scores or values to be thresholded.
    threshold : float
        Threshold value; elements > `threshold` are assigned label 1.
