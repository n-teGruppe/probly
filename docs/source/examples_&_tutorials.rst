...
These tutorials are the worked counterparts to :ref:`advanced_topics` and are meant to be read together.
In addition to transformations and ensembles, :ref:`advanced_topics` also introduces hierarchical and mixture-model recipes; corresponding worked examples are included below.
...
This step is an uncertainty-aware transformation in the sense of :ref:`advanced_topics` (Custom Transformations / design principles).
...
The number of samples and batching are typical “advanced knobs”; see :ref:`advanced_topics` (Performance & Computational Efficiency, Reproducibility & Randomness).
...
This is the practical efficiency pattern from :ref:`advanced_topics` (working under performance/memory constraints via subsetting).
...
This example corresponds to the mixed-model workflow pattern in :ref:`advanced_topics` (Advanced Usage Patterns & Integration concepts).
...
Training multiple members is where large-model considerations (batching, runtime, memory) from :ref:`advanced_topics` become relevant.
...
Together, Tutorials (I)–(V) cover the main advanced recipes referenced in :ref:`advanced_topics`: transformations, ensembles (including mixed), hierarchical models, and mixture models.
